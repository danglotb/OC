...
object MainOnLineRandom extends App {
  ...
  val solutions = Build500Random.build500RandomOnLine(Nil, Nil,instance,0)
...
object MainOffLineRandom extends App {
  ...
  val instance = tspBuilder.build2mTsp("input/tsp/randomA100.tsp", "input/tsp/randomB100.tsp")
    ...
    val solutions = Build500Random.build500Random(Nil, instance)

  //build data
  var str = ""
  val dominators =  Filter2o.filter(solutions)
  dominators.foreach { x =>
      str += x._1 + "\t" + x._2 + "\n"
  }
  data.Util.write("data/domRandom500mTSP.dat", str)

  str = ""
  (solutions.diff(dominators)).foreach { x =>
    str += x._1 + "\t" + x._2 + "\n"
  }
  data.Util.write("data/random500mTSP.dat", str)
